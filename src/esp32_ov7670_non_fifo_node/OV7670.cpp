...
{
  vsyncOk = false;
  ClockEnable(XCLK, 10000000); //slower XCLK helps no-FIFO OV7670 wiring/debug

  DEBUG_PRINT("Waiting for VSYNC...");
  pinMode(VSYNC, INPUT);
  unsigned long vsyncStart = millis();
  while(!digitalRead(VSYNC) && millis() - vsyncStart < 3000);
  while(digitalRead(VSYNC) && millis() - vsyncStart < 3000);
  vsyncOk = millis() - vsyncStart < 3000;
  if (!vsyncOk)
  {
    Serial.println(" VSYNC timeout - check OV7670 VSYNC, XCLK, power and GND");
  }
  DEBUG_PRINTLN(" done");
  ...
  Serial.println("Camera test pattern enabled");
  testImage();
#endif
